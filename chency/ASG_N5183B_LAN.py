...
start_freq = float(input(Back.WHITE + Fore.BLUE + "Enter Starting Frequency (GHz): "))
stop_freq = float(input(Back.WHITE + Fore.BLUE + "Enter Last Frequency (GHz): "))
N = int(input(Back.WHITE + Fore.BLUE + "How many points: "))
...
for freq in arange(start_freq, stop_freq, (stop_freq - start_freq)/N):
    ...
    feq = asg.query(":FREQ?")
    sleep(1)
    print(Fore.YELLOW + "%s. frequency %s" %(i, feq))
    i += 1
